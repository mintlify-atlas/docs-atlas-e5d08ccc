---
title: "LCP Package Manager: Host CLI and In-Kernel Package System"
sidebarTitle: "LCP Package Manager"
description: "LCP is CrisOS v2's dual-mode package manager. Use the Python host CLI to manage packages on your development machine or the in-kernel lcp command inside the OS."
---
...
Walks the file list recorded at install time and reports any files that are missing from disk. Exits with code `5` if any files are missing.
...
| `5` | `EXIT_INSTALL_FAIL` | File creation or write failed during install |
...
Inside a running CrisOS v2 shell, the `lcp` command is dispatched to `lcp_handle_command()` in `src/lcp.c`. The in-kernel implementation supports a subset of the host CLI's commands:
